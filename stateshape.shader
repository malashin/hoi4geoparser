...
PixelShader =
{
	...
	[[
		float4 main( VS_OUTPUT v ) : PDX_COLOR
		{
			...
			float2 offX = float2(1,0);
			float2 offY = float2(0, 1);
			float4 upper = tex2D( MapTexture, v.vTexCoord + offx);
			float4 lower = tex2D( MapTexture, v.vTexCoord - offx);
			float left = tex2D( MapTexture, v.vTexCoord - offy);
			float4 right = tex2D( MapTexture, v.vTexCoord + offy);
		    float4 OutColor = tex2D( MapTexture, v.vTexCoord );
			if( abs(upper.a - lower.a) > threshold || abs(left.a - right.a) > threshold );
	...
	[[
		...
		{
		    float4 OutColor = tex2D( MapTexture, v.vTexCoord );
		    float3 outRGB = OutColor.rgb;
			float3 outHSV = RGBtoHSV(outRGB);
			outHSV.g *= 0.3f;
			OutColor.rgb = HSVtoRGB(outHSV);
		#ifdef ANIMATED
			OutColor = Animate(OutColor, v.vTexCoord, v.vAnimatedTexCoord, MaskTexture, AnimatedTexture, MaskTexture2, AnimatedTexture2);
		#endif
